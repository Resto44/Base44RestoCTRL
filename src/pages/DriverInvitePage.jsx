 ...
 * 2. Call acceptDriverInvite (unauthenticated) → get invite preview OR instant success
 * 3. If pending_auth → show "Sign In" button → store token → redirect to login
 * 4. On return from login → poll isAuthenticated() → call acceptDriverInvite again → redirect to /driver
 */
...
export default function DriverInvitePage() {
  ...
  const startFlow = async () => {
    ...
    if (!token) {
      setErrorMsg('No invitation token found in the link. Please use the original invite link.');
      setState('error');
      return;
    }

    log(`Token: ${token.slice(0, 8)}…`);
    const storedToken = sessionStorage.getItem('pending_driver_invite_token') || localStorage.getItem('pending_driver_invite_token');
    const returningFromLogin = storedToken === token;

    let authed = await base44.auth.isAuthenticated();
    log(`Auth: ${authed}, returningFromLogin: ${returningFromLogin}`);
    ...
    if (authed) {
      sessionStorage.removeItem('pending_driver_invite_token');
      localStorage.removeItem('pending_driver_invite_token');
    }
  ...
  const callAcceptInvite = async (authed) => {
    ...
    log(`Calling acceptDriverInvite (authed=${authed})…`);

    const res = await base44.functions.invoke('acceptDriverInvite', { token });
    const data = res?.data;
    ...
    if (data.pending_auth) {
      sessionStorage.setItem('pending_driver_invite_token', token);
      setInviteData(data);
  ...
  const handleLogin = () => {
    ...
    sessionStorage.setItem('pending_driver_invite_token', token);
    localStorage.setItem('pending_driver_invite_token', token);
      ...
      <div className="w-full max-w-sm">
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div className="bg-gradient-to-br from-blue-600 to-blue-800 px-6 pt-8 pb-6 text-center">
            ...
            <h1 className="text-white font-black text-xl">Driver Portal</h1>
            <p className="text-blue-200 text-sm mt-1">Restaurant Manager Pro</p>
          </div>
          ...
          <div className="p-6 text-center">
            ...
            {state === 'pending_auth' && (
              <div className="space-y-5">
                ...
                <div>
                  ...
                  {inviteData?.driver_name && (
                    <p className="text-slate-600 mt-1">Hello, <strong>{inviteData.driver_name}</strong>!</p>
                  )}
                </div>
                {(inviteData?.restaurant_name || inviteData?.branch_label) && (
                  <div className="bg-blue-50 rounded-2xl p-4 text-left space-y-1.5">
                    ...
                    <p className="text-sm text-slate-600">🚴 Role: <strong>Delivery Driver</strong></p>
                    {inviteData.email && (
                ...
                <p className="text-sm text-slate-500">
                  Sign in or create your account to activate the driver portal.
                </p>
                <Button className="w-full h-12 text-base font-bold rounded-2xl" onClick={handleLogin}>
                  Sign In / Create Account
                </Button>
                <p className="text-xs text-slate-400">
                  Use the same email address to sign up — your driver role will be assigned automatically.
                </p>
              </div>
            )}
            ...
            {state === 'success' && (
              <div className="space-y-4">
                ...
                <p className="text-sm text-slate-500">Redirecting to your driver dashboard…</p>
        ...
        <p className="text-center text-xs text-slate-500 mt-4">
          Restaurant Manager Pro · Secure Driver Onboarding
        </p>
      </div>
    </div>
  );
