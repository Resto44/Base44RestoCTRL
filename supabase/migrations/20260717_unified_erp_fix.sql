-- ============================================================
-- Unified ERP Architecture Fix
-- Date: 2026-07-17
-- Fixes:
--   1. erp_decide_membership now also populates branch_assignments
--   2. Public read on restaurants (active only) for registration dropdown
--   3. Public read on branches (active only) for registration dropdown
--   4. Ensure profiles.organization_id is kept in sync with restaurant_id
-- ============================================================

-- ── 1. Fix erp_decide_membership to also populate branch_assignments ──────────
...
BEGIN
  ...
  -- ── KEY FIX: Populate branch_assignments on approval ─────────────────────────
  IF p_decision = 'approved' AND final_branch IS NOT NULL THEN
    INSERT INTO public.branch_assignments (
      user_id, branch_id, organization_id, role, assigned_by, is_primary, active
    ) VALUES (
      result.user_id, final_branch, actor.restaurant_id, result.role, auth.uid(), TRUE, TRUE
    )
...
-- ── 2. Allow anon/public to read active restaurants for registration dropdown ──
-- (The existing "Allow public insert for registration" policy covers INSERT.
--  We need SELECT for the registration dropdown.)
...
-- ── 3. Allow anon/public to read active branches for registration dropdown ─────
...
-- ── 4. Ensure organization_id stays in sync with restaurant_id in profiles ─────
-- Backfill existing profiles where organization_id is NULL but restaurant_id is set
...
GRANT EXECUTE ON FUNCTION public.erp_decide_membership TO authenticated;
